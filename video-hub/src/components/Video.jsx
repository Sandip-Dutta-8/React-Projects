...
const Videos = () => {
    ...
    const lectureDescription = [
        ...
        'Lecture 7 description',
    ];

    const [videoSrc, setVideoSrc] = useState(videosArr[0]);
    const [lectureNum, setLectureNum] = useState(0);
    const [Description, setDescription] = useState(lectureDescription[0]);
        ...
        <Stack direction={['column', 'row']} h={'100vh'}>
            <VStack w={'full'}>
                ...
                <VStack alignItems={'flex-start'} p={'5'} w={'full'} overflowY={"auto"}
                    ...
                    <Heading>Lecture {lectureNum + 1}</Heading>
                    <Text>
                        {Description}
                    </Text>
                </VStack>
                ...
                {videosArr.map((item, index) => (
                    <Button
                        ...
                        onClick={() => {
                            setVideoSrc(item)
                            setLectureNum(index)
                            setDescription(lectureDescription[index])
                        }}
